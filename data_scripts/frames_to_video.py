...
Usage:
  python frames_to_video.py /path/to/frames 24
"""
...
def main():
    ...
    p.add_argument("--input_dir", type=str, help="Directory containing image frames")
    p.add_argument("--fps", type=float, help="Frames per second (e.g., 24 or 30)")
    args = p.parse_args()
    ...
    # Output video path = <dir_basename>.mp4 (next to the frames dir)
    out_name = os.path.basename(os.path.normpath(args.input_dir)) or "output"
    out_path = os.path.join(os.path.dirname(os.path.normpath(args.input_dir)), f"{out_name}.mp4")

    # 'mp4v' is widely supported; if you specifically want H.264, use ffmpeg (see note below).
